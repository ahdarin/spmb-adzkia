...
<body class="bg-adzkia-bg antialiased text-adzkia-dark w-full m-0 p-0 flex flex-col min-h-screen overflow-x-hidden">
    ...
    {{-- MODAL PROGRAM STUDI --}}
    <div x-data x-show="$store.kampus.modalOpen" class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:px-4" style="display: none;" x-cloak>
        {{-- Backdrop --}}
        ...
        {{-- Modal Panel: full-width sheet di mobile, card di sm+ --}}
        <div x-show="$store.kampus.modalOpen" 
            ...
            <div class="p-6 sm:p-10">
                {{-- Handle bar untuk mobile (bottom sheet visual cue) --}}
                ...
                <div class="flex justify-between items-start mb-5 sm:mb-6">
                    <div class="flex-1 pr-3">
                        <h2 class="text-2xl sm:text-3xl font-extrabold text-adzkia-dark tracking-tight" x-text="$store.kampus.activeProgram.nama_prodi"></h2>
                        <span class="mt-2 sm:mt-3 inline-block px-3 py-1 bg-adzkia-badge-bg text-adzkia-badge-txt text-[11px] font-extrabold rounded-full uppercase" x-text="$store.kampus.activeProgram.akreditasi"></span>
                    </div>
                    <button @click="$store.kampus.closeModal()" class="p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
                ...
                <div class="space-y-5 sm:space-y-6">
                    <div>
                        ...
                        <p class="text-gray-600 font-medium leading-relaxed text-[14px] sm:text-[15px]" x-text="$store.kampus.activeProgram.deskripsi"></p>
                    </div>
                    <div class="bg-adzkia-bg p-4 sm:p-6 rounded-2xl border border-gray-100">
                        ...
                        <div class="flex items-baseline gap-2 flex-wrap">
                            ...
                            <span class="text-xl sm:text-2xl font-black text-adzkia-dark" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format($store.kampus.activeProgram.biaya || 0)"></span>
                            <span class="text-xs text-gray-500">/semester</span>
                        </div>
                    </div>
                    <a href="/register" class="w-full py-3.5 sm:py-4 bg-adzkia-dark text-white font-extrabold rounded-full shadow-xl hover:scale-105 active:scale-95 transition-all flex justify-center items-center text-[14px] sm:text-base">
                        Daftar Program Ini
                    </a>
                </div>
            </div>
