...
{{-- ============================================================
     ...
     ============================================================ --}}
<section class="px-4 sm:px-6 lg:px-16 py-4 sm:py-6 lg:py-8 bg-adzkia-bg"
    x-data="{
        activeSlide: 0,
        slides: [
            ...
            { image: 'https://images.unsplash.com/photo-1523580846011-d3a5bc25702b?q=80&w=1280&h=720&auto=format&fit=crop', title: 'Pendaftaran Mahasiswa Baru {{ $globalSetting->tahun_akademik ?? '2025/2026' }}', subtitle: 'Dapatkan Beasiswa Adzkia Unggul hingga 100% untuk Siswa Berprestasi' },
            { image: 'https://images.unsplash.com/photo-1562774053-701939374585?q=80&w=1280&h=720&auto=format&fit=crop', title: 'Fasilitas Modern & Terpadu', subtitle: 'Lingkungan belajar yang nyaman didukung laboratorium berstandar industri' }
        ],
        ...
        <div class="relative w-full aspect-[4/3] sm:aspect-[16/9] lg:aspect-[21/9] rounded-[1.25rem] sm:rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden shadow-2xl border-2 sm:border-4 border-white">
            <template x-for="(slide, index) in slides" :key="index">
                <div x-show="activeSlide === index"
                    ...
                    <div class="absolute bottom-8 left-5 right-14 sm:bottom-10 sm:left-10 sm:right-20 lg:bottom-16 lg:left-16 lg:right-16 text-white">
...
<main class="px-4 sm:px-6 lg:px-16 py-8 sm:py-10 lg:py-12 flex flex-col md:flex-row items-center md:justify-between bg-adzkia-bg">
    <div class="w-full md:w-1/2 md:pr-12 mb-8 md:mb-0">
        <div class="inline-block px-4 py-1.5 bg-adzkia-badge-bg text-adzkia-badge-txt text-[13px] font-bold rounded-full tracking-wide mb-4 sm:mb-6">
            TAHUN AKADEMIK {{ $globalSetting->tahun_akademik ?? '2025/2026' }}
        </div>

        {{-- Badge gelombang aktif --}}
        @if($gelombangAktif)
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-200 rounded-xl text-[12px] font-bold text-emerald-700 mb-4 ml-2">
            <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
            {{ $gelombangAktif->nama_gelombang }} · Dibuka s/d {{ \Carbon\Carbon::parse($gelombangAktif->tanggal_selesai)->translatedFormat('d M Y') }}
        </div>
        @endif
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-5">
            ...
            <a href="{{ $globalSetting->brosur_path ? asset('uploads/docs/' . $globalSetting->brosur_path) : '#' }}"
               target="_blank"
               class="px-8 py-4 text-adzkia-blue text-[15px] font-bold rounded-full flex items-center justify-center gap-2 hover:bg-adzkia-badge-bg transition-colors border border-adzkia-blue/20 sm:border-transparent">
...
{{-- ============================================================
     SECTION 3: VIDEO PROFIL — thumbnail dari YouTube via setting
     ============================================================ --}}
<section class="px-4 sm:px-6 lg:px-16 py-12 sm:py-16 lg:py-24 bg-white flex flex-col md:flex-row items-start md:items-center md:justify-between border-t border-gray-50"
         x-data="{ openVideo: false }">
    <div class="w-full md:w-5/12 md:pr-12 mb-8 md:mb-0">
        ...
        <p class="text-gray-500 text-[14px] sm:text-[15px] leading-relaxed mb-6 sm:mb-8">
            Universitas Adzkia Padang adalah hasil konversi STKIP Adzkia Padang menjadi Universitas. Terdiri dari 18 Program Studi dan terakreditasi Institusi dari BAN-PT.
        </p>
    ...
    <div class="w-full md:w-7/12 md:flex md:justify-end">
        @if($videoId)
        {{-- Thumbnail asli dari YouTube --}}
        <div @click="openVideo = true"
             class="relative w-full md:max-w-[650px] aspect-video rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden shadow-2xl group cursor-pointer border-2 sm:border-4 border-white">
            <img src="{{ $thumbnail }}"
                 onerror="this.src='https://i.ytimg.com/vi/{{ $videoId }}/hqdefault.jpg'"
                 class="w-full h-full object-cover transition duration-700 group-hover:scale-105"
                 alt="Video Profil Universitas Adzkia">
            <div class="absolute inset-0 bg-adzkia-blue/20 group-hover:bg-adzkia-blue/40 transition duration-500 flex items-center justify-center">
                <div class="w-14 h-14 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-300">
                    <div class="w-0 h-0 border-t-[8px] border-t-transparent border-l-[16px] border-l-adzkia-red border-b-[8px] border-b-transparent ml-1.5"></div>
                </div>
            </div>
            <div class="absolute bottom-4 left-4 bg-black/50 backdrop-blur-sm text-white text-[12px] font-bold px-3 py-1.5 rounded-lg">
                ▶ Video Profil Universitas Adzkia
            </div>
        </div>
        @else
        <div class="relative w-full md:max-w-[650px] aspect-video rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden shadow-2xl border-2 sm:border-4 border-white bg-gray-100 flex items-center justify-center">
            <p class="text-gray-400 font-bold text-[14px]">Video profil belum dikonfigurasi</p>
        </div>
        @endif
    </div>
    ...
    <div x-show="openVideo" x-cloak
         class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 sm:p-6"
        ...
        <div class="relative w-full max-w-4xl md:aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl">
            <button @click="openVideo = false"
                    class="absolute top-3 right-3 sm:top-4 sm:right-4 z-10 p-2 bg-white/20 hover:bg-adzkia-red transition-colors rounded-full text-white">
                <i data-feather="x"></i>
            </button>
            <iframe x-show="openVideo"
                    class="w-full h-full min-h-[250px] sm:min-h-[400px]"
                    :src="openVideo ? 'https://www.youtube.com/embed/{{ $videoId }}?autoplay=1' : ''"
...
<section id="prodi" class="px-4 sm:px-6 lg:px-16 py-10 sm:py-14 lg:py-20 bg-adzkia-bg">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6 mb-6 sm:mb-10 lg:mb-12">
        ...
        <a href="/program-studi"
           class="p-5 sm:p-6 lg:p-8 bg-white border border-gray-100 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group flex flex-col justify-between min-h-[210px] sm:min-h-[240px] lg:min-h-[260px]">
            <div>
                ...
                <h3 class="text-base sm:text-xl font-extrabold text-adzkia-blue mb-2 group-hover:text-adzkia-red transition-colors">{{ $prodi->nama }}</h3>

            </div>
            <p class="text-[13px] font-extrabold text-adzkia-red flex items-center gap-2 group-hover:gap-3 transition-all mt-4">
...
{{-- ============================================================
     SECTION 5: BANNER AI REKOMENDASI
     ============================================================ --}}
<section class="w-full py-8 sm:py-12 px-4 sm:px-6 lg:px-16 max-w-7xl mx-auto">
    <div class="bg-gradient-to-r from-adzkia-blue to-blue-600 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 lg:p-14 text-white relative overflow-hidden shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 sm:gap-8">
        ...
        <div class="relative z-10 w-full md:max-w-2xl text-center md:text-left">
            ...
            <h2 class="text-2xl sm:text-3xl lg:text-4xl font-black mb-3 sm:mb-4 leading-tight">Bingung Pilih Program Studi?</h2>
            <p class="text-blue-100 text-[13px] sm:text-[15px] font-medium leading-relaxed mb-6 sm:mb-8">
                Jawab beberapa pertanyaan singkat mengenai minat dan bakatmu, dan biarkan sistem kami merekomendasikan program studi yang paling cocok untuk masa depanmu!
            </p>
...
<section id="fasilitas" class="px-4 sm:px-6 lg:px-16 py-12 sm:py-16 lg:py-24 bg-adzkia-blue text-white">
    <div class="text-center max-w-2xl mx-auto mb-8 sm:mb-12 lg:mb-16">
        ...
        <p class="text-blue-100 text-[13px] sm:text-[15px] font-medium leading-relaxed">
            Kami menyediakan lingkungan belajar yang nyaman dan mendukung kreativitas serta inovasi mahasiswa.
        </p>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
        <div class="relative h-[220px] sm:h-[240px] lg:h-[260px] rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden group cursor-pointer border-2 border-white/10 hover:border-white/30 transition-colors">
            <img src="https://images.unsplash.com/photo-1562774053-701939374585?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" class="w-full h-full object-cover transition duration-700 group-hover:scale-110" alt="Laboratorium">
            <div class="absolute inset-0 bg-gradient-to-t from-adzkia-blue via-adzkia-blue/40 to-transparent opacity-90"></div>
            <div class="absolute bottom-6 sm:bottom-8 left-6 sm:left-8"><h3 class="text-[1.1rem] sm:text-[1.3rem] font-extrabold text-white group-hover:text-red-300 transition-colors">Laboratorium Terpadu</h3></div>
        </div>
        <div class="relative h-[220px] sm:h-[240px] lg:h-[260px] rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden group cursor-pointer border-2 border-white/10 hover:border-white/30 transition-colors">
            <img src="https://images.unsplash.com/photo-1541339907198-e08756dedf3f?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" class="w-full h-full object-cover transition duration-700 group-hover:scale-110" alt="Perpustakaan">
            <div class="absolute inset-0 bg-gradient-to-t from-adzkia-blue via-adzkia-blue/40 to-transparent opacity-90"></div>
            <div class="absolute bottom-6 sm:bottom-8 left-6 sm:left-8"><h3 class="text-[1.1rem] sm:text-[1.3rem] font-extrabold text-white group-hover:text-red-300 transition-colors">Perpustakaan Digital</h3></div>
        </div>
        <div class="relative h-[220px] sm:h-[240px] lg:h-[260px] rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden group cursor-pointer border-2 border-white/10 hover:border-white/30 transition-colors sm:col-span-2 md:col-span-1">
            <img src="https://images.unsplash.com/photo-1497366216548-37526070297c?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" class="w-full h-full object-cover transition duration-700 group-hover:scale-110" alt="Coworking">
            <div class="absolute inset-0 bg-gradient-to-t from-adzkia-blue via-adzkia-blue/40 to-transparent opacity-90"></div>
            <div class="absolute bottom-6 sm:bottom-8 left-6 sm:left-8"><h3 class="text-[1.1rem] sm:text-[1.3rem] font-extrabold text-white group-hover:text-red-300 transition-colors">Area Diskusi & Lounge</h3></div>
        </div>
    </div>
</section>
            ...
            <a href="/berita/{{ $item->slug }}" class="group bg-white rounded-2xl sm:rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all cursor-pointer flex flex-col">
                <div class="relative w-full h-44 sm:h-48 overflow-hidden bg-gray-100">
                    ...
                    <img src="{{ $item->thumbnail ? asset('uploads/berita/' . $item->thumbnail) : 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?q=80&w=500' }}"
                         alt="{{ $item->judul }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                </div>
                <div class="p-4 sm:p-6 flex flex-col flex-grow">
                    <p class="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mb-2 sm:mb-3">
                        {{ \Carbon\Carbon::parse($item->created_at)->translatedFormat('d F Y') }}
                    </p>
                    <h3 class="text-base sm:text-lg font-extrabold text-adzkia-blue group-hover:text-adzkia-red transition-colors mb-2 sm:mb-4 line-clamp-2">{{ $item->judul }}</h3>
                    <p class="text-[13px] text-gray-500 font-medium line-clamp-2 mb-3 sm:mb-4">{{ $item->ringkasan }}</p>
    ...
    <div class="max-w-[800px] mx-auto">
        ...
        <div class="space-y-3 sm:space-y-4">
            ...
            <div x-data="{ open: false }"
                 class="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 cursor-pointer hover:border-adzkia-blue transition-colors"
                 @click="open = !open">
            ...
            @empty
            <div class="text-center text-gray-500 font-bold py-4">Belum ada data FAQ.</div>
            @endforelse
        </div>
    </div>
</section>
        ...
        <div class="space-y-8 sm:space-y-10 lg:space-y-12 pr-0 md:pr-8">
            <div>
                ...
                <p class="text-gray-500 font-medium leading-relaxed text-[14px] sm:text-[15px]">
                    Tim admisi kami siap membantu menjawab pertanyaan Anda seputar proses pendaftaran, jadwal seleksi, hingga beasiswa.
                </p>
            </div>
            <div class="space-y-6 sm:space-y-8">
                <div class="flex gap-4 sm:gap-6 items-start group">
                    <div class="w-11 h-11 sm:w-12 sm:h-12 bg-adzkia-badge-bg rounded-2xl flex items-center justify-center shrink-0 mt-1 group-hover:bg-adzkia-blue transition-colors duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 text-adzkia-blue group-hover:text-white transition-colors duration-300"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                    </div>
                    <div>
                ...
                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $globalSetting->telepon ?? '6281234567890') }}" target="_blank" class="flex gap-4 sm:gap-6 items-start group cursor-pointer">
                    <div class="w-11 h-11 sm:w-12 sm:h-12 bg-adzkia-badge-bg rounded-2xl flex items-center justify-center shrink-0 mt-1 group-hover:bg-adzkia-blue transition-colors duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 text-adzkia-blue group-hover:text-white transition-colors duration-300"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
                    </div>
                    <div>
                ...
                <a href="mailto:{{ $globalSetting->email ?? 'pmb@adzkia.ac.id' }}" class="flex gap-4 sm:gap-6 items-start group cursor-pointer">
                    <div class="w-11 h-11 sm:w-12 sm:h-12 bg-adzkia-badge-bg rounded-2xl flex items-center justify-center shrink-0 mt-1 group-hover:bg-adzkia-blue transition-colors duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 text-adzkia-blue group-hover:text-white transition-colors duration-300"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                    </div>
                    <div>
